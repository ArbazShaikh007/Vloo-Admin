...
const Index = () => {
  ...
  // 👇 fallback from localStorage if context is false or undefined (after refresh)
  const savedLogin = JSON.parse(localStorage.getItem("Login Response") || "null");

  const finalIsSubadmin =
    typeof is_subadmin === "boolean"
      ? is_subadmin
      : savedLogin?.is_subadmin === true ||
        savedLogin?.is_subadmin === 1 ||
        savedLogin?.is_subadmin === "1";

  console.log("🔎 ServiceProvider → is_subadmin (context):", is_subadmin);
  console.log("🔎 ServiceProvider → finalIsSubadmin (with localStorage):", finalIsSubadmin);
  ...
  const [ProviderDataslist, setProviderDataslist] = useState();
  ...
  const BannerData = async (search) => {
    ...
    // ✅ DECIDE THE ENDPOINT HERE (always fresh)
    const endpoint = finalIsSubadmin
      ? "/admin_view/provider_list"
      : "/admin_view/main_provider_list";
    ...
    try {
      ...
      console.log("🚀 ~ Response:provider_list", Response);
      setProviderDataslist(Response.data.user_list);
      setProviderDatas(Response.data);
      setloading(false);
    } catch (error) {
  ...
  useEffect(() => {
    if (reloadProviderList === true) {
      BannerData();
      setreloadProviderList(false);
    }
  }, [reloadProviderList]);
  ...
  const handlePageClick = (pageNumber) => {
    if (
      pageNumber >= 1 &&
      pageNumber <= ProviderDatas?.pagination_info?.total_pages
    ) {
  ...
  const renderPaginationItems = () => {
    const totalPages = ProviderDatas?.pagination_info?.total_pages || 1;
  ...
  const HandleEditmodel = (item) => {
    setEditprovidermodel(true);
    setSelectedprovider(item);
  };

  const HandleDeletemodel = (item) => {
    setDeleteprovidermodel(true);
    setSelectedprovider(item);
  };
        ...
        <div className="All-Containor-perfect-second-divv">
          <div className="heding_div">
            <div className="addbtn_div">
              <NavLink className="secondary_btn" onClick={setAddprovidermodel}>
  {finalIsSubadmin ? "Add Workers" : "Add Service Provider"}
</NavLink>
          ...
          <div className="tabell_bo table_body_user">
            <MDBTable align="middle">
              ...
              <MDBTableBody className="table_body">
                {ProviderDataslist?.map((item, index) => {
                  return (
                    <tr key={index}>
                      <td>{index + 1}</td>
                      <td>
                        <div className="d-flex align-items-center">
                          <img
                            src={item.image ? item.image : defultImg}
                            alt=""
          ...
          <div className="pagination_div">
            <div className="totoalpage_show_div">
              <p>Total Pages: {ProviderDatas?.pagination_info?.total_pages}</p>
            </div>
            <nav aria-label="Page navigation example">
              <MDBPagination className="mb-0">
                ...
                <MDBPaginationItem
                  disabled={
                    activePage === ProviderDatas?.pagination_info?.total_pages
                  }
                >
                  <MDBPaginationLink
                    onClick={() => handlePageClick(activePage + 1)}
